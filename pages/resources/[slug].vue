    ...
    <section class="bg-primary-50">
        <div class="max-w-[1440px] px-4 lg:px-20 m-auto py-16">
            <div class="max-w-[900px] m-auto mb-16">
                <p v-if="data.article.article_type == 'blogs'" class="text-neutral-600 mb-4">Blogs</p>
                <p v-if="data.article.article_type == 'case-studies'" class="text-neutral-600 mb-4">Case Studies</p>
                <h1 class="text-3xl lg:text-5xl font-bold mb-4">{{ data.article.title }}</h1>
                <div
                    ...
                    <p>{{ formatDate(data.article.publish_date) }}</p>
            ...
            <div class="max-w-[1032px] m-auto">
                ...
                <div v-if="data.suggestedArticles.length > 0" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <nuxt-link v-for="item in data.suggestedArticles" :key="item.slug"
...
const { get } = useApi();
const { data: data } = await get(`/articles/${route.params.slug}`);
